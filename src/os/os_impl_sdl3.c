...
os_display_t* os_display_create(mem_allocator_t* alloc, i32 width, i32 height)
{
    ...
    display->window = SDL_CreateWindow("Terrarium", width, height, 0);
...
bool os_event_poll(os_event_t* out)
{
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_EVENT_MOUSE_MOTION:
            ...
            out->u.mousemove.pos_x = (i32)event.motion.x;
            out->u.mousemove.pos_y = (i32)event.motion.y;
            out->u.mousemove.rel_x = (i32)event.motion.xrel;
            out->u.mousemove.rel_y = (i32)event.motion.yrel;
            return true;

        case SDL_EVENT_MOUSE_BUTTON_DOWN:
        case SDL_EVENT_MOUSE_BUTTON_UP:
            out->type = event.type == SDL_EVENT_MOUSE_BUTTON_DOWN ? OS_EVENT_MOUSEBUTTON_DOWN : OS_EVENT_MOUSEBUTTON_UP;
            out->u.mousebutton.pos_x = (i32)event.button.x;
            out->u.mousebutton.pos_y = (i32)event.button.y;
        ...
        case SDL_EVENT_MOUSE_WHEEL:
            ...
            out->u.mousewheel.pos_x = (i32)event.wheel.mouse_x;
            out->u.mousewheel.pos_y = (i32)event.wheel.mouse_y;
            out->u.mousewheel.scroll_x = event.wheel.integer_x;
            out->u.mousewheel.scroll_y = event.wheel.integer_y;
            return true;

        case SDL_EVENT_KEY_DOWN:
        case SDL_EVENT_KEY_UP:
            out->type = event.type == SDL_EVENT_KEY_DOWN ? OS_EVENT_KEY_DOWN : OS_EVENT_KEY_UP;
            out->u.key.is_repeat = event.key.repeat;
            out->u.key.code = scancode_to_keycode(event.key.scancode);
        ...
        default:
            continue; // Skip and process the next one so we don't block
        }
    }
...
static os_key_code_e scancode_to_keycode(SDL_Scancode scancode)
{
    ...
    ASSERT(scancode < SDL_SCANCODE_COUNT, "Invalid scancode: %d\n", scancode);

    static const os_key_code_e table[SDL_SCANCODE_COUNT] = {
